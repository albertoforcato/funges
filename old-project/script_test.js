  ...
  try {
    topPredictions.forEach((pred, rank) => {
      ...
      if (!CLASS_NAMES[modelIndex] || !CLASS_NAMES[modelIndex][classIndex]) {
        console.error(
          `❌ ERROR: CLASS_NAMES[${modelIndex}][${classIndex}] does not exist!`,
          { modelIndex, classIndex }
        );
    ...
    position => {
      ...
      map.once('idle', () => {
        ...
        features.forEach((f, i) => {
          ...
          const scores = Object.entries(f.properties).filter(
            ([k, v]) => k.endsWith('_score') && typeof v === 'number' && v > 6
          ); // limit for finding species. CHange here
          ...
          if (scores.length) {
            ...
            scores.forEach(([key, value]) => {
              ...
              const edibleName =
                speciesDisplayMap[speciesCode] ||
                speciesCode.replace(/_/g, ' ');
        ...
        const sortedItems = Object.entries(foundItems).sort(
          (a, b) => b[1] - a[1]
        );
        ...
        if (sortedItems.length === 0) {
          intro.innerHTML =
            "🍃 Dear forager, the season is tough at the moment—nothing seems to be growing around you. But don't worry, better times will come. Stay tuned for more edibles to be added soon!";
          listContainer.appendChild(intro);
        } else {
          intro.innerHTML =
            '🌿 Hey fellow forager! Following edibles could be growing in your proximity:';
          listContainer.appendChild(intro);
          ...
          const selectedKeys = sortedItems.map(
            ([item]) =>
              Object.entries(speciesDisplayMap).find(
                ([code, name]) => name === item
              )?.[0] || item
          );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', event => {
    if (
      ...
      !predictionBox.contains(event.target)
    ) {
